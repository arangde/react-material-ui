...
class MemberPoints extends React.Component {
  ...
  handleEdit(id) {
    // this.props.push(`/admin/points/${id}`)
  }

  handleAdd = () => {
    // this.props.push(`/admin/points/create`)
  }
            ...
            <CardBody>
              <div className={classes.tableResponsive}>
                <Table className={classes.table}>
                  ...
                  <TableBody>
                    {points.sort(this.getSorting(order, orderBy))
                      .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                      .map(point => {
                        const isSelected = this.isSelected(point.id);
                        return (
                          <TableRow
                            hover
                            role="checkbox"
                            ...
                            selected={isSelected}
                          >
                            <TableCell className={classes.tableCell}>{point.old_point}</TableCell>
                            <TableCell className={classes.tableCell}>{point.new_point}</TableCell>
                            <TableCell className={classes.tableCell}>{point.note}</TableCell>
                            {editAndRemove ? (
                              <TableCell className={classes.tableActions}>
                                <IconButton
                                  aria-label="Edit"
                                  className={classes.tableActionButton}
                                  onClick={() => this.handleEdit(point.id)}
                                >
                                  <EditIcon
                                ...
                                <IconButton
                                  aria-label="Close"
                                  className={classes.tableActionButton}
                                  onClick={() => this.handleRemove(point.id)}
                                >
                                  <CloseIcon
